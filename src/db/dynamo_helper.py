...
class DynamoDBService:
    ...
    def remove_empty_string(self, obj):
        if isinstance(obj, dict):
            return {k: self.remove_empty_string(v) for k, v in obj.items() if k not in ['', None]}
        elif isinstance(obj, list):
            return [element for element in [self.remove_empty_string(v) for v in obj] if element is not None]
    ...
    def get_item(self, key, index=None):
        query = {
            "Key": key
        }
        if index:
    ...
    def get_batch_items(self, key_name=None, key_values=None, sort_key_name=None, sort_key_value=None, keys_dicts=None):
        keys = keys_dicts if keys_dicts else [{key_name: value} for value in key_values]
        items = []
        for keys_chunk in chunks(keys, 25):
            params = {"RequestItems": {self.table_name: {"Keys": list(keys_chunk)}}}
            response = self.dynamodb.batch_get_item(**params)
            responses = response.get("Responses", {})
            items.extend(responses.get(self.table_name, []))
